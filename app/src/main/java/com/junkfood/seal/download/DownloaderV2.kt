...
class DownloaderV2Impl(private val appContext: Context) : DownloaderV2, KoinComponent {
    ...
    override fun enqueue(task: Task) {
        taskStateMap[task] = Task.State(Idle, null, Task.ViewState(url = task.url, title = task.url))
    }
            ...
            .launch(Dispatchers.Default) {
                ...
                DownloadUtil.downloadVideo(
                    ...
                    progressCallback = { progressPercentage, _, text ->
                        ...
                        // Only update state if 500ms have passed to avoid overwhelming the UI
                        if (currentTime - lastUpdateTime > 500) {
                            val progress = progressPercentage / 100f
                            if (downloadState is Running) {
                                // Update on main thread for safety
                                runBlocking(Dispatchers.Main.immediate) {
                                    downloadState = (downloadState as Running).copy(progress = progress, progressText = text)
                                }
    ...
    private fun Task.execute() {
        // This function is for custom commands, which we have removed from the UI.
        // The logic can remain here as it is not currently reachable in our simplified app.
    }
}